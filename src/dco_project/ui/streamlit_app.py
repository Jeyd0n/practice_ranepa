"""Streamlit UI for the dynamic pricing pipeline."""
...
def run_app() -> None:  # pylint: disable=too-many-locals
    ...
    with st.form("pricing_form"):
        ...
        if use_mocks_only:
            ...
            plan_sales_month = st.number_input(
                "План продаж за месяц (лоты)",
                min_value=1,
                max_value=500,
                value=40,
            )
            fact_sales_month = st.number_input(
                "Факт продаж за месяц (лоты)",
                min_value=0,
                max_value=500,
                value=38,
            )
            unsold_share = st.slider(
    ...
    st.metric(
        "Рекомендованная цена квартиры, ₽",
        f"{result.recommended_total_price:,.0f}".replace(",", " "),
    )
